...
import { normalizeIcons, normalizeIcon, STANDARD_VIEWBOX } from "@/lib/icon-utils";
...
const bundleNameSchema = z.string().min(1).max(80).describe("Bundle name");

const bundleDescriptionSchema = z
  .string()
  .max(300)
  .optional()
  .nullable()
...
const bundleIconIdsSchema = z
  .array(iconIdSchema)
  .min(1)
  .max(200)
  .describe("List of icon IDs to save in the bundle");

const strokePresetSchema = z
  ...
  create_my_bundle({ name: "dashboard", iconIds: ["lucide:home", "lucide:settings"] })`,
      ...
      async (params) => {
        ...
        const output = {
          bundle: {
            id: bundle.id ?? "",
            name: bundle.name ?? params.name,
            description: bundle.description ?? null,
            icon_count: bundle.icon_count ?? bundleIcons.length,
            is_public: bundle.is_public ?? false,
            share_slug: bundle.share_slug ?? null,
          },
        };

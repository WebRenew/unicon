...
function generateVueComponents(icons) {
  const components = icons.map((icon) => {
    const name = toPascalCase(icon.normalizedName);
    const attrs = getSvgAttributes(icon);
    return `<!-- ${name}.vue -->
...
function generateSvelteComponents(icons) {
  const components = icons.map((icon) => {
    const name = toPascalCase(icon.normalizedName);
    const attrs = getSvgAttributes(icon);
    return `<!-- ${name}.svelte -->
...
program.command("bundle").description("Bundle icons into a file (or directory with --split)").option("-q, --query <query>", "Search query").option("-c, --category <category>", "Filter by category").option("-s, --source <source>", "Filter by source (lucide, phosphor, hugeicons)").option("-f, --format <format>", "Output format: react, vue, svelte, svg, json", "react").option("-o, --output <path>", "Output file/directory path").option("-l, --limit <number>", "Maximum number of icons", "100").option("--split", "Write each icon to its own file (requires -o as directory)").action(async (options) => {
